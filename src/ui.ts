...
<meta name="color-scheme" content="dark"><title>Sidecar · Conversations</title><link rel="icon" href="data:,">
...
<aside class="sidebar" aria-label="Conversations">
  ...
  <footer class="sidebar-footer"><span class="lock" aria-hidden="true">↔</span><div>Encrypted conversations<span id="relay-name">Your configured relay</span></div></footer>
</aside>
...
export const uiCss = `
:root{color-scheme:dark;font-family:ui-monospace,"SFMono-Regular",Menlo,Consolas,monospace;font-size:13px;color:var(--ink);background:var(--base);font-synthesis:none;--base:#1d2021;--panel:#282828;--raised:#32302f;--line:#504945;--ink:#ebdbb2;--soft:#bdae93;--muted:#a89984;--amber:#d79921;--blue:#83a598;--purple:#d3869b}
*{box-sizing:border-box}body{margin:0}button,input,select{font:inherit;color:inherit}button{cursor:pointer}button:disabled{cursor:wait;opacity:.6}button:focus-visible,input:focus-visible,select:focus-visible,summary:focus-visible{outline:2px solid var(--amber);outline-offset:3px}h1,h2,h3,p{margin:0}[hidden]{display:none!important}::selection{background:#665c54;color:#fbf1c7}*{scrollbar-width:thin;scrollbar-color:#665c54 transparent}
...
.brand{height:73px;display:flex;align-items:center;gap:10px;padding:0 8px;border-bottom:1px solid var(--line);font-weight:700;font-size:19px;letter-spacing:-.7px}.brand-mark{color:var(--amber);font-size:21px;letter-spacing:-3px;padding-right:4px}.local-label{margin-left:auto;font-size:10px;font-weight:400;color:var(--muted);letter-spacing:0}.local-label:before{content:'['}.local-label:after{content:']'}
.identity{display:flex;align-items:center;gap:10px;padding:18px 8px 22px}.identity strong{display:block;font-size:12px;font-weight:500;color:var(--ink)}.identity span{display:block;color:var(--muted);font-size:10px;margin-top:5px}.avatar{display:grid;place-items:center;width:26px;height:26px;flex:none;border:1px solid var(--line);color:var(--amber);font-size:10px}.avatar.alt{color:var(--blue)}
...
function renderConversation(messages) {
  ...
  for (const message of messages.filter(m => m.type !== 'reaction')) {
    ...
    if (message.type === 'ack') { const event = el('div','event'); event.append(el('time','',time(message.ts)),el('span','',name(message.from) + ' accepted the task')); container.append(event); continue; }
